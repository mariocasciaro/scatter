 ...
 * @returns {Module}
 */
Container.prototype.registerModule = function(name, rawMod, annotations, component) {
 ...
 * @returns {Module}
 */
Container.prototype.registerModuleInstance = function(name, instance, annotations, component) {
  if(this.modules[name]) {
    throw new Error("Duplicate module name found: " + name);
  }
...
Container.prototype.load = function(deps, fromModule, initOption) {
  var self = this;
  var fromName = fromModule ? fromModule.name : "NONE";
  self.log('debug', "Resolving dependencies " + util.inspect(deps) + " for module " +
    fromName);
