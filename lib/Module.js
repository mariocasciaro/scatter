...
Module.prototype.wire = function() {
  ...
  return self.wirePromise = self.instancePromise.then(function() {
...
Module.prototype.initialize = function() {
  ...
  var initializePromise = self.wirePromise.then(function() {
    ...
    if(_.isFunction(initializeFoo)) {
      ...
      return self.invokeInject(initializeFoo, initialize[0], self.instance, true);
    }
    return undefined;
  }).then(function() {
...
Module.prototype.initializeTree = function() {
  var self = this;
  if(self.initStatus === Constants.MODULE_TREE_INITIALIZED) {
    return Promises.resolve(self);
  }

  var deepDeps = self.getUnititializedModuleDependenciesDeep();
  self.log('silly', "Ensuring dependencies are initialized for module " + self.name + " " + deepDeps);
  
   var promises = [];
  //initialize the parent
  if(self.parent) {
    promises.push(self.parent.initializeTree());
  }
  promises.push(self.initialize());
  
  _.each(deepDeps, function(mod) {
    if(mod.initStatus < Constants.MODULE_INITIALIZED) {
      promises.push(mod.initialize());
    }
  });
  
  return Promises.all(promises).then(function(results) {
...
Module.prototype.getUnititializedModuleDependencies =  function(deps) {
  var self = this;
  var newDeps = {};
  deps = deps || newDeps;
  
  function addDep(dep) {
    if(self.isModuleDep(dep) && deps[dep] === void 0 && dep !== self.name) {
      deps[dep] = self.getModuleFromDep(dep);
      newDeps[dep] = deps[dep];
    }
  }
  
  _.each(this.descriptor.args, addDep);
  _.each(this.descriptor.properties, addDep);
  if(this.descriptor.initialize) {
    _.each(this.descriptor.initialize[0], addDep);
  }
...
Module.prototype.getUnititializedModuleDependenciesDeep = function(deepDeps) {
  var newDeps = this.getUnititializedModuleDependencies(deepDeps);
  deepDeps = deepDeps || newDeps;
  _.each(newDeps, function(module) {
    if(module.initStatus < Constants.MODULE_TREE_INITIALIZED) {
      module.getUnititializedModuleDependenciesDeep(deepDeps);
    }
  });
...
Module.prototype.load = function(deps, ensureInitialized) {
  return this.container.load(deps, this, ensureInitialized);
};
...
Module.prototype.invokeInject = function(foo, deps, context, ensureInitialized) {
  var self = this;
  return self.load(deps, ensureInitialized).then(function(modDeps) {
