...
function Contribution(container, name, fromModule) {
...
Contribution.prototype.sequence = function(/* varargs */) {
  var args = Array.prototype.slice.apply(arguments);
  return this._invoke(args);
};

Contribution.prototype.invoke = Contribution.prototype.sequence;

Contribution.prototype.pipeline = function(initial) {
  return this._invoke(initial, {chained: true});
};

Contribution.prototype.any = function() {
 ...
 * @private
 */
Contribution.prototype._invoke = function(args, options) {
  var self = this;
  options = options || {};
  self.log('debug', "Invoking service " + self.name + ", resolving modules...");
 ...
 *
 */
Contribution.prototype.getProviderModules = function() {
  var self = this;
  ...
  _.each(modules, function(mod, moduleName) {
    ...
    _.each(mod.descriptor.provides, function(provideDesc, providedContributionName) {
      if(providedContributionName === self.serviceName) {
...
Contribution.prototype._loadModules = function(names) {
  var self = this;
  var retModules = {};
 ...
 * @private
 */
Contribution.prototype._invokeWithModules = function(args, options, modules) {
  var self = this;
  var initialInput;
  var results;
  if(options.chained) {
  ...
  self.log('debug', "Invoking service " + self.name + " over modules " + _.pluck(modules, 'name'));
  ...
  _.each(modules, function(mod) {
    promise = promise.then(function(prevRes) {
      ...
      self.log('debug', "Invoking service " + self.name + " over module " + mod.name);
      var argsToUse = args;
      if(options.chained) {
        argsToUse = [prevRes];
      }
  ...
  return promise.then(function(res) {
    self.log('trace', "All services " + self.name + " were invoked");
    if(options.chained || options.oneResult) {
      return res;
    }
    return results;
 ...
 * Topological Sort
 */
Contribution.prototype._sortProviders = function(modules) {
  var self = this;
