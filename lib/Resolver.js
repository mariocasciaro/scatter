...
Resolver.prototype.expandPaths = function(globs) {
  ...
  _.each(globs, function(globbedPath) {
    ...
    _.each( expandedPaths, function(possibleRoot) {
      var root = self._createRoot(possibleRoot, true);
      if(root) {
        roots.push(root);
      }
    });
...
Resolver.prototype.discoverRoots = function(possibleRoot) {
  ...
  var root = self._createRoot(possibleRoot);
  if(root) {
