...
function Resolver(options) {
  this.roots = this.expandPaths(options.roots, true);
  ...
  if(this.nodeModulesRoot) {
    //add npm roots (shallow search)
    this.expandPaths([path.join(this.nodeModulesRoot,'/*')], false);
  }
 ...
 * (with descriptor)
 */ 
Resolver.prototype.expandPaths = function(globs, ignoreMissingDescriptor, basePath) {
  var self = this;
  var roots = [];
...
Resolver.prototype._parseRoot = function(possibleRoot, ignoreMissingDescriptor) {
  ...
  if(fs.existsSync(descFile)) {
    ...
    if(desc) {
      ...
      if(roots) {
        return this.expandPaths(roots, true, possibleRoot);
      } else {
        return [{
          root: possibleRoot,
          descriptor: desc
