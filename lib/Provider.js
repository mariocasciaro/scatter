...
function Provider(container, name, fromModule) {
...
Provider.prototype.sequence = function(/* varargs */) {
  var args = Array.prototype.slice.apply(arguments);
  return this._invoke(args);
};

Provider.prototype.pipeline = function(initial) {
  return this._invoke(initial, {chained: true});
};

Provider.prototype.any = function() {
 ...
 * @private
 */
Provider.prototype._invoke = function(args, options) {
  var self = this;
  options = options || {};
  ...
  if(containers) {
    ...
    return self.container.getProviderModules(self.name, self.container.isStateful).then(function(moduleList) {
      return self.loadModules(moduleList).then(function(modules) {
...
Provider.prototype.loadModules = function(names) {
  var self = this;
  var retModules = {};
  ...
  _.each(names, function(modName) {
    promise = promise.then(function() {
      return self.container.loadModule(modName, self.fromModule);
    }).then(function(mod) {
 ...
 * @private
 */
Provider.prototype._invokeWithModules = function(args, options, modules) {
  var self = this;
  var initialInput;
  var results;
  if(options.chained) {
 ...
 * Topological Sort
 */
Provider.prototype._sortProviders = function(modules) {
  var self = this;
  ...
  _.each(providerModules, function(name) {
    var desc = modules[name].getProviderDescriptor(self.serviceName);
